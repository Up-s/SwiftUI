...
class KeyboardObserver: ObservableObject {
  ...
  func parse(notification: Notification) -> Context? {
    ...
    let safe = UIApplication.shared.windows.first?.safeAreaInsets
    let animatioDuration = (userInfo[UIResponder.keyboardAnimationDurationUserInfoKey] as? NSNumber)?.doubleValue ?? 0.25
    ...
    if let value = userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
      ...
      if frame.origin.y < UIScreen.main.bounds.height {
        height = frame.height - (safe?.bottom ?? 0)
      }
    }
    
    return Context(animationDuration: animatioDuration, height: height)
  }
}
